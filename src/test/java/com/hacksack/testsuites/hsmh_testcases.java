...
//  @Test(priority=2)
  public void verifyFindDoctor() {
	  ...
	  if (findDoctor.isDisplayed()) {
	      System.out.println("Find a Doctor link is visible.");
	  } else {
	      System.out.println("Find a Doctor link not visible.");
	  }
	  findDoctor.click();

	// Check if navigated to doctor search page
...
//  @Test(priority=3)
  public void HoverClickHospitals() throws InterruptedException {
	...
	// Find the “Find a Location” element
	WebElement findLocation = driver.findElement(By.xpath("//span[contains(text(),'Find a Location')]"));
	...
	// Click on Hospitals link under the dropdown
	WebElement hospitalsLink = driver.findElement(By.linkText("Hospitals"));
	hospitalsLink.click();

	System.out.println("Navigated to Hospitals page. URL: " + driver.getCurrentUrl());
  }
  public void SearchWithValidInput() throws InterruptedException {
	  WebElement searchBox = driver.findElement(By.id("keyword-search")); // adjust locator
	  searchBox.sendKeys("Cardiology");

	  WebElement searchButton = driver.findElement(By.xpath("//button[contains(text(),'Search Doctors')]"));
	  searchButton.click();

	  // Wait or validate results loaded
	  Thread.sleep(3000);
